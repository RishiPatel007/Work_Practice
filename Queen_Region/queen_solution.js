...
function printSolutionMatrix(matrix) { 
    ...
    for (let row of matrix) {
        console.log(row.map(cell => cell === null ? emptyCell : cell.toString().padStart(cellWidth)).join(' | '));
...
function addColorFromBoundry(matrix, cell) {
	...
	for ([x, y] of boundries) {
		if (x >= 0 && x < gridSize && y >= 0 && y < gridSize && matrix[x][y] != null) {
...
function getRandomBoard() {
	...
	let colors = ["green", "blue", "red", "white", "yellow" , "purple" , "navy" , "silver" , "magenta"];
...
function startGame() {
	...
	let edgeOccupied = [];
	while(true){
		gridSize = parseInt(readline.question("Please enter grid size between 5 to 9 : "))
